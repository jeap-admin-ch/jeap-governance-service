...
class ComponentConsumesSignedMessagesRuleTest {

    @Mock
    private PromTimeSeriesQueryRepository promTimeSeriesQueryRepository;

    @InjectMocks
    private ComponentConsumesSignedMessagesRule rule;
    ...
    void metadata() {
        ...
        assertThat(metadata.ruleId().id()).isEqualTo("component-consumes-signedmessages");
        assertThat(metadata.label()).isEqualTo("Component Consumes Signed Messages");
    }
    ...
    void notConsumer() {
        ...
        //then
        assertThat(result.state()).isEqualTo(State.OK);
        assertThat(result.stateComment()).isEqualTo("Service is not a messaging consumer");
    }
    ...
    void ok() {
        ...
        when(promTimeSeriesQueryRepository.findBy(PromQueryType.JEAP_MESSAGING_TOTAL, 1L))
                .thenReturn(List.of(PromTimeSeries.builder()
                        ...
                        .sample(new PromTimeSeriesSample(Map.of("foo", "bar"), List.of("123")))
                        .build()));
        when(promTimeSeriesQueryRepository.findBy(PromQueryType.JEAP_MESSAGING_SIGNATURE_REQUIRED, 1L))
                .thenReturn(List.of(PromTimeSeries.builder()
                        .prometheusQueryType(PromQueryType.JEAP_MESSAGING_SIGNATURE_REQUIRED)
                        .queryTimestamp(ZonedDateTime.now())
                        .sample(new PromTimeSeriesSample(Map.of("foo", "bar"), List.of("123")))
                        .build()));
        ...
        //then
        assertThat(result.state()).isEqualTo(State.OK);
        assertThat(result.stateComment()).isEqualTo("Message signature enforcement enabled");
    }
    ...
    void signatureNotEnforced() {
        ...
        when(promTimeSeriesQueryRepository.findBy(PromQueryType.JEAP_MESSAGING_TOTAL, 2L))
                .thenReturn(List.of(PromTimeSeries.builder()
                        .prometheusQueryType(PromQueryType.JEAP_MESSAGING_TOTAL)
                        .queryTimestamp(ZonedDateTime.now())
                        .sample(new PromTimeSeriesSample(Map.of("foo", "bar"), List.of("0")))
                        .build()));
        when(promTimeSeriesQueryRepository.findBy(PromQueryType.JEAP_MESSAGING_SIGNATURE_REQUIRED, 2L))
                .thenReturn(List.of(PromTimeSeries.builder()
                        .prometheusQueryType(PromQueryType.JEAP_MESSAGING_SIGNATURE_REQUIRED)
                        .queryTimestamp(ZonedDateTime.now())
                        .sample(new PromTimeSeriesSample(Map.of("foo", "bar"), List.of("0")))
        ...
        assertThat(result.stateComment()).isEqualTo("Signature is not enforced for consumption");
    }
